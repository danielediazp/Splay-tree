...
void SplayTree::pre_order_vector(Node* root, std::vector<Node*> &target_vector, int x, int y, sf::RenderWindow *window, char type, float scale, std::vector<positionalNode> &positional_nodes, int depth) {
    ...
    if (type == 'l') {
        ...
        if (root->right_ != nullptr) {
            ...
            right_p[1].position = sf::Vector2f(x + 50 + scale/8, y + scale + depth);
            window->draw(right_p);
        }
    } else if (type == 'r') {
        if (root->left_ != nullptr) {
            sf::VertexArray left_p(sf::LinesStrip, 2);
            left_p[0].position = sf::Vector2f(x, y);
            left_p[1].position = sf::Vector2f(x - 50 - scale/8, y + scale + depth);
            window->draw(left_p);
        }
        if (root->right_ != nullptr) {
            sf::VertexArray right_p(sf::LinesStrip, 2);
...
//
        if (root->right_ != nullptr) {
            sf::VertexArray right_p(sf::LinesStrip, 2);
            right_p[0].position = sf::Vector2f(x,y);
            right_p[1].position = sf::Vector2f((x + 50 + scale/3), (y + scale/3 + depth));
            window->draw(right_p);
        }
...
//
    ...
    window->draw(value);
    if (type == 'l') {
        pre_order_vector(root->left_, target_vector, x - 50 - scale, y + scale + depth, window, 'l', scale, positional_nodes, depth + 50);
        pre_order_vector(root->right_, target_vector, x + 50 + scale/8, y + scale + depth, window, 'x', scale, positional_nodes, depth + 50);//

    } else if (type == 'r') {
        pre_order_vector(root->left_, target_vector, x - 50 - scale/8, y + scale + depth, window, 'x', scale, positional_nodes, depth + 50);
        pre_order_vector(root->right_, target_vector, x + 50 + scale, y + scale + depth, window, 'r', scale, positional_nodes, depth + 50);//
    }
    else if (type == 'x') {
        pre_order_vector(root->left_, target_vector, (x - 50 - scale/3), (y + scale/3 + depth), window, 'x', scale/2, positional_nodes, depth + 50);
        pre_order_vector(root->right_, target_vector, (x + 50 + scale/3), (y + scale/3 + depth), window, 'x', scale/2, positional_nodes, depth + 50);//
    }
    else {
        pre_order_vector(root->left_, target_vector, x - 50 - scale, y + scale + depth, window, 'l', scale, positional_nodes, depth + 50);
        pre_order_vector(root->right_, target_vector, x + 50 + scale, y + scale + depth, window, 'r', scale, positional_nodes, depth + 50);//
